...
def test_the_engine_hands_its_button_to_settlement() -> None:
    """Every settlement test here calls settle directly, so all of them passed
    while the engine forwarded no button at all."""
    import inspect

    from app.game import hand_engine

    source = inspect.getsource(hand_engine)
    assert "self.button," in source.split("settle(")[1].split(")")[0], (
        "the engine must pass its button through to settlement"
    )
