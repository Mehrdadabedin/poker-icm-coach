...
def next_button(current: int, num_seats: int, active_seats: set[int]) -> int:
    ...
    If the current button seat is no longer active (e.g. eliminated), the
    button moves to the next active seat clockwise so the tournament can
    continue after an elimination.
    """
